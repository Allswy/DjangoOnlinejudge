...
class Problem(BaseModel):
    ...
    modify_time = models.DateTimeField(_('modify time'), default=now)
    status = models.CharField(
        _('status'),
        max_length=1,
        choices=STATUS_CHOICES,
...
class ProblemTag(Tag):
    ...
    def get_Problem_count(self):
        return Problem.objects.filter(tags__name=self.name).distinct().count()

class OnlinejudgeSettings(models.Model):
    """blog的配置"""
